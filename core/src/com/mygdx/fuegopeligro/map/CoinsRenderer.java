...
import com.mygdx.fuegopeligro.physics.CoinsBodyFactory;

/**
 * @author JDEsguerra
 */
public class CoinsRenderer implements Telegraph {
    ...
    public CoinsRenderer(final float unitScale) {
        this.collectibles = new Array<Entity>();
        this.removed = new ObjectSet<Entity>(64);
        this.unitScale = unitScale;
        MessageManager.getInstance().addListener(this, MessageType.COINS_COLLECTED.code());
    }
    ...
    public void load(final World world, final BodyEditorLoader loader, final AssetManager assets,
        ...
        for (MapObject mo : layer.getObjects()) {
            ...
            if (CARROT_TYPE.equals(mo.getProperties().get(TYPE_PROPERTY, CARROT_TYPE, String.class))) {
                bodyFactory = new CoinsBodyFactory(loader);
                entity = EntityFactory.createCoins(world, assets);
            } else {
    ...
    public boolean handleMessage(final Telegram msg) {
        Collectible coins = (Collectible) msg.extraInfo;
        removed.add(coins);
        return true;
    }
}
